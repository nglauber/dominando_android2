    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null) {
            ...
            if (acao != null) {
                ...
                } else if (FavoritosWidget.ACAO_SITE.equals(acao)){
                    int appWidgetId = intent.getIntExtra(
                            AppWidgetManager.EXTRA_APPWIDGET_ID,
                            AppWidgetManager.INVALID_APPWIDGET_ID);
                    int posicao = widgetStates.get(appWidgetId);
                    String site = sites[posicao];
                    Intent it = new Intent(Intent.ACTION_VIEW, Uri.parse("http://" + site));
